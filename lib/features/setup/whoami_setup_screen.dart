...
class WhoAmISetupScreen extends StatefulWidget {
  ...
  @override
  State<WhoAmISetupScreen> createState() => _WhoAmISetupScreenState();
}
...
class _WhoAmISetupScreenState extends State<WhoAmISetupScreen> {
    ...
    for (var frame = 0; frame < 12; frame++) {
      ...
      setState(() {
        _animatedCode =
            List.generate(
              finalCode.length,
              (index) => alphabet[(frame * 9 + index * 13) % alphabet.length],
            ).join();
      });
    }
  }
  ...
  Future<void> _startGame() async {
    ...
    Navigator.of(context).pushNamed(AppRouter.whoamiGamePath, arguments: party);
  }
            ...
            child: Column(
              ...
              children: [
                ...
                if (dirtyWordsEnabled)
                  ...
                  Text(
                    'Используются только обычные слова. Более смелые варианты можно включить в настройках.',
                    style: Theme.of(context).textTheme.bodyMedium,
                  ),
              ],
            ),
